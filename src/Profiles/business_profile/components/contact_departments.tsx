...
import { ContactDepartment } from "@/Profiles/types"
import { formatPhoneNumberSimple, saveContactWithContactsAPI, saveContactEnhanced } from "@/utils/contact-utils"
import { Phone } from "lucide-react"
import Image from "next/image"
...
export const ContactDepartmentCard = ({ title, phone, whatsapp, telegram, telephone, sms }: ContactDepartment) => {
	...
	const handleSaveContact = async () => {
		...
		try {
			await saveContactWithContactsAPI(title, phone)
		} catch (error) {
			console.error("Contacts API failed, falling back:", error)
			await saveContactEnhanced(title, phone)
		} finally {
			setIsSaving(false)
		}
	}
			...
			<div className='mt-4 flex items-center justify-between border-t border-slate-100 pt-3 dark:border-white/5'>
				...
				<div className='flex gap-2'>
					<button
						...
						aria-label={isSaving ? "Saving contact..." : "Save to contacts"}>
						{isSaving ? (
							<>
